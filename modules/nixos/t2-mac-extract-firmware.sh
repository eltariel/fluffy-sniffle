...
work_dir="$(mktemp -d)"
out_dir="${out}/lib/firmware"

mkdir -pv "${out_dir}"

tar --warning=no-unknown-keyword -xvC "${work_dir}" -f "${archive}"

python3 $0 "${work_dir}" "${work_dir}/firmware-renamed.tar"

tar -xvC "${out_dir}" -f "${work_dir}/firmware-renamed.tar"
rm -r "${work_dir}"
